...
RUN /usr/bin/python3 -m pip install --no-cache-dir --upgrade pip setuptools wheel && \
    /usr/bin/python3 -m pip install --no-cache-dir pysam pandas biopython && \
    /usr/bin/python3 -c "import pysam, pandas, Bio; print('system Python packages OK')"
...
RUN /opt/conda/bin/python3 -m pip install --no-cache-dir pysam pandas biopython && \
    /opt/conda/bin/python3 -c "import pysam; print('conda base pysam OK')"
...
RUN set -eux; \
  refdir=/ducks4/ressources/reference; \
  ln -sf "$refdir/hg38_no_alt.fa" "$refdir/Homo_sapiens_GRCh38_no_alt.fasta"; \
  ln -sf "$refdir/hg38_no_alt.fa.fai" "$refdir/Homo_sapiens_GRCh38_no_alt.fasta.fai"; \
  ln -sf "$refdir/chm13v2.0.fa" "$refdir/T2T-chm13v2.0.fa"; \
  ln -sf "$refdir/chm13v2.0.fa.fai" "$refdir/T2T-chm13v2.0.fa.fai"
